...
//                    else 
    ...
    public void         testBoundPoly () {
        ...
        DXTickDB                db = getTickDb();
        {
            ...
            try {
                rcd = (RecordClassDescriptor) Introspector.introspectSingleClass(MarketMessage.class);
            } catch (Introspector.IntrospectionException e) {
                e.printStackTrace();
            }
            StreamOptions options = StreamOptions.fixedType(StreamScope.DURABLE, "S1", "", 0, rcd);
            DXTickStream s1 = db.createStream ("S1", options);
