...
import deltix.timebase.messages.service.RealTimeStartMessage;
...
//    }
    ...
    @Test(timeout = 20000)
    public void testLive() throws InterruptedException, Introspector.IntrospectionException {

        final DXTickStream historicStream = createBarsStream("bars", 0);

        populateStream(historicStream, new BarsGenerator(new GregorianCalendar(2010, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100, "MSFT", "AAPL"));
        ...
        Thread liveStreamProducer = new Thread() {
            @Override
            public void run() {
                populateStream(liveStream, new BarsGenerator(null, (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100, "Q", "W") {
                    int counter = 0;
                    @Override
                    public boolean next() {
                        counter++;
    ...
    @Test(timeout = 60000)
    public void testLive1() throws InterruptedException, Introspector.IntrospectionException {
        ...
        final DXTickStream bars = createStream("bars", options);

        BarsGenerator gn = new BarsGenerator(
                new GregorianCalendar(2010, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100, "MSFT", "AAPL");
        ...
        final DXTickStream live = createStream("live", options);
        ...
        Thread producer = new Thread() {
            ...
            public void run() {
                BarsGenerator gn =
                    new BarsGenerator(new GregorianCalendar(), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100, "QQQQ", "WWWW");
...
//                assert message instanceof RealTimeStartMessage;
            ...
            if (message instanceof deltix.timebase.api.messages.BarMessage && ((deltix.timebase.api.messages.BarMessage) message).getVolume() == 1 && count > 92)
                break;
...
//                        "WWWW".equals(message.getSymbol().toString());
    ...
    @Test(timeout = 60000)
    public void testLive2() throws InterruptedException, Introspector.IntrospectionException {
        ...
        Thread producer = new Thread() {
            ...
            public void run() {
                BarsGenerator gn =
                    new BarsGenerator(null, (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, -1,
                            "QQQQ", "WWWW", "DDDD");
    ...
    @Test
    public void testCursor() throws InterruptedException, Introspector.IntrospectionException {
        DXTickDB tickdb = getTickDb();
        ...
        RecordClassDescriptor rd1 = (RecordClassDescriptor) Introspector.introspectSingleClass(BarMessage.class);
        RecordClassDescriptor rd2 = (RecordClassDescriptor) Introspector.introspectSingleClass(TradeMessage.class);

        options.setPolymorphic(rd1, rd2);

        final DXTickStream data = createStream("data", options);

        BarsGenerator gn = new BarsGenerator(
                new GregorianCalendar(2010, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 10, "MSFT", "AAPL");
...
//        } while (cursor.next() && count < 10);
            ...
            assertEquals(REAL_TIME_START_MESSAGE_DESCRIPTOR, cursor.getCurrentType());
    ...
    @Test(timeout = 20000)
    public void testLiveHist() throws InterruptedException, Introspector.IntrospectionException {
        ...
        TickCursor cursor1 = bars.select(Long.MIN_VALUE, options1, null, (CharSequence[]) null);
        ...
        Thread producer = new Thread() {
            ...
            public void run() {
                BarsGenerator gn =
                    new BarsGenerator(null, (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100, "QQQQ", "WWWW");
    ...
    @Test(timeout = 20000)
    public void testLiveTruncate() throws InterruptedException, Introspector.IntrospectionException {
        ...
        DXTickStream stream = createStream("zzz", so);
        ...
        TickCursor cursor = stream.select(Long.MIN_VALUE, options, null, (CharSequence[]) null);
    ...
    private DXTickStream        createTestStream(DXTickDB db, int count, String name) throws Introspector.IntrospectionException {

        RecordClassDescriptor classDescriptor = (RecordClassDescriptor) Introspector.introspectSingleClass(BarMessage.class);

        StreamOptions options = StreamOptions.fixedType(StreamScope.DURABLE, name, null, 0, classDescriptor);
        options.location = "abc"; //TODO
        DXTickStream stream = createStream(name, options);

        BarsGenerator gn = new BarsGenerator(
                new GregorianCalendar(2012, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, count,
                "MSFT", "AAPL", "ORCL", "IBM");
    ...
    @Test
    public void testQQL() throws Introspector.IntrospectionException {
        DXTickDB tickDb = getTickDb();
        ...
        final DXTickStream bars = createStream(options.name, options);
    ...
    public void testTruncate() throws Introspector.IntrospectionException {
        RecordClassDescriptor classDescriptor = (RecordClassDescriptor) Introspector.introspectSingleClass(BarMessage.class);
        ...
        BarsGenerator gn = new BarsGenerator(
                new GregorianCalendar(2012, 0, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_DAY, 100, "IBM");

        populateStream(stream, gn);

        long time = stream.getTimeRange()[1];
        time -= deltix.timebase.api.messages.BarMessage.BAR_DAY;

        SelectionOptions options = new SelectionOptions(false, true);
        options.realTimeNotification = true;
        TickCursor cursor = stream.select(0, options, null, (CharSequence[]) null);
    ...
    @Test(timeout = 60000)
    public void testTransient() throws InterruptedException, Introspector.IntrospectionException {
        RecordClassDescriptor classDescriptor = (RecordClassDescriptor) Introspector.introspectSingleClass(BarMessage.class);
        ...
        DXTickDB db = getTickDb();
        final DXTickStream stream = createStream("transient", options);
        ...
        Thread consumer = new Thread() {
            @Override
            public void run() {
                TickCursor cursor = stream.select(0, new SelectionOptions(false, true), null, (CharSequence[]) null);
    ...
    public void testPerformance() throws Introspector.IntrospectionException {
        final int total = 10000000;
        DXTickStream stream = createTestStream(getTickDb(), total, "perf");

        try (TickCursor cursor = stream.select(Long.MIN_VALUE, new SelectionOptions(true, true), null, (CharSequence[]) null)) {
    ...
    private static void populateStream(DXTickStream data, BarsGenerator gn) {
    ...
    private static TickCursor selectLive(DXTickStream liveStream, long timestamp) {
        ...
        return liveStream.select(timestamp, options2, null, (CharSequence[]) null);
    }

    private DXTickStream createBarsStream (String streamKey, int distributionFactor) throws Introspector.IntrospectionException {
        StreamOptions options = new StreamOptions (StreamScope.DURABLE, "dummy name for " + streamKey, null, distributionFactor);
        options.location = "abc"; //TODO
        options.setFixedType((RecordClassDescriptor) Introspector.introspectSingleClass(BarMessage.class));

        return createStream(streamKey, options);
    }
}
