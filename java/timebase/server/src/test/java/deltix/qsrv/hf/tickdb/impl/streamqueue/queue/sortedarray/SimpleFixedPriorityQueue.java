package deltix.qsrv.hf.tickdb.impl.streamqueue.queue.sortedarray;
 ...
 */
public class SimpleFixedPriorityQueue<T extends TimeStampedMessage> {
    ...
    public SimpleFixedPriorityQueue(Collection<MessageSource<T>> sources) {
        ...
        for (int i = 0; i < size; i++) {
            this.timestamps[i] = this.sources[i].getMessage().getNanoTime();
        }
    }
    ...
    public void offer(MessageSource<T> obj) {
        ...
        long newNanoTime = obj.getMessage().getNanoTime();
    ...
    private void insertWithShift(MessageSource<T> objToInsert, long newNanoTime, MessageSource<T>[] sources, long[] timestamps) {
