...
import deltix.qsrv.hf.pub.codec.CachingCodecMetaFactory;
...
import deltix.util.concurrent.FrequencyLimiter;
import deltix.util.concurrent.QuickExecutor;
import deltix.util.concurrent.UncheckedInterruptedException;
import deltix.util.io.AbstractDataStore;
import deltix.util.io.CachedLocalIP;
...
//            TickDBImpl.LOGGER.warning ("STREAM: " + key + "; LOCATION: " + options.location);
    ...
    static DXTickStream             read (File file) {
        try {
            ...
            throw new deltix.util.io.UncheckedIOException ("Error reading file: " + file, ex);
        }
    }
    ...
    static DXTickStream             read (AbstractPath path) {
        try {
            ...
            throw new deltix.util.io.UncheckedIOException ("Error reading file: " + path.getPathString(), ex);
        }
    }
    ...
    synchronized void                     saveToFile (boolean holdsRemoteStreamLock) {
        ...
        if (metadataLocation.isLocal()) {
            ...
            try {
                ...
                throw new deltix.util.io.UncheckedIOException(ex);
            }
//            try {
...
//            }
    ...
    public synchronized void        open (boolean readOnly) {
        ...
        try {
            ...
            Util.close(fileLock);
            throw new deltix.util.io.UncheckedIOException ("Failed to open stream [" + key + "]", x);
        }
            ...
            try {
                ...
                if (!isReadOnly || vf.exists()) {
                    versionsData = new StreamVersionsContainer(this, getCodecFactory(false),
                        new Class[] {StreamTruncatedMessage.class, MetaDataChangeMessage.class },
                        new RecordClassDescriptor[] {Messages.STREAM_TRUNCATED_MESSAGE_DESCRIPTOR, Messages.META_DATA_CHANGE_MESSAGE_DESCRIPTOR} );
                }
