...
import deltix.util.jcg.scg.*;
import deltix.util.lang.CompilationExceptionWithDiagnostic;
...
public class JavaBeanGenerator extends BeanGenerator {
    ...
    protected String        getClassNameFor (ClassDescriptor cd) {
        ...
        return (lang == Language.JAVA) ?
                (doEscapeIdentifierForJava (dot < 0 ? cdName : cdName.substring (dot + 1))) :
                (doEscapeIdentifierForCS (dot < 0 ? cdName : cdName.substring (dot + 1)));
    }
