package deltix.qsrv.hf.tickdb.shell;
...
import deltix.qsrv.hf.tickdb.testframework.TBLightweight;
...
import deltix.qsrv.hf.tickdb.replication.*;
...
import deltix.qsrv.hf.tickdb.tool.StreamComparer;
...
import deltix.timebase.messages.service.MetaDataChangeMessage;
...
//    }
    ...
    public void         testFileStream() throws Throwable {
        ...
        final File fileName = Home.getFile("/testdata/uhf/messageFiles/securities.qsmsg.gz");
        //TimebaseTestUtils.createFileStream(TickDBCreator.LOCATION, "securities", fileName.getCanonicalPath());
        ...
        try {
            ...
            DXTickStream stream = db.createFileStream("securities", fileName.getCanonicalPath());
            ...
            new StreamReplicator().replicate(from, to, new ReplicationOptions());
            compareStreams(from.getSource(), to.getSource());
        } finally {
            runner.shutdown();
        }
...
//    }
    ...
    @Test
    public void         testDFBackup() throws Throwable {

        final File file1 = Home.getFile("/testdata/uhf/messageFiles/securities5.qsmsg.gz");
        //final File file2 = Home.getFile("testdata\\tickdb\\misc\\securities_120303.qsmsg.gz");
        ...
        try {
            ...
            Home.getFile("temp/backup").mkdirs();
            final StreamStorage from = new StreamStorage(db, "securities");
            final FileStorage to = new FileStorage(Home.getFile("temp/backup"));

            ReplicationOptions rOptions = new ReplicationOptions();
            //rOptions.entities = new IdentityKey[] {stream.listEntities()[0]};
            rOptions.entities = new IdentityKey[] {new ConstantIdentityKey("AAPL")};
            new StreamReplicator().replicate(from, to, rOptions);

            StreamStorage test = new StreamStorage(db, "testDFBackup");
            new StreamReplicator().restore(to, test, new RestoreOptions("securities"));
            compareStreams(from.getSource(), test.getSource(), rOptions.entities);
//
//        importFile(file2, stream);
//
//        new StreamReplicator().replicate(from, to, new ReplicationOptions());
//        compareStreams(from.getSource(), to.getSource(), stream.listEntities());
    ...
    @Test
    public void         testDDF() throws Throwable {

        final File file1 = Home.getFile("/testdata/uhf/messageFiles/securities5.qsmsg.gz");
        final File file2 = Home.getFile("/testdata/tickdb/misc/securities5_120303.qsmsg.gz");

        ServerRunner runner = new ServerRunner(false, true);
        try {
            runner.startup();
            ...
            StreamOptions   options =
                    StreamOptions.polymorphic(
                            StreamScope.DURABLE, "securities", null, 5,
                            StreamConfigurationHelper.mkSecurityMetaInfoDescriptors()
                    );
            ...
            options.distributionFactor = 5;
            options.name = "testDDF";
            db.createStream(options.name, options);

            final StreamStorage from = new StreamStorage(db, "securities");
            final StreamStorage to = new StreamStorage(db, "testDDF");
    ...
    @Test
    public void         testDDFSymbols() throws Throwable {

        final File file1 = Home.getFile("/testdata/uhf/messageFiles/securities5.qsmsg.gz");
        final File file2 = Home.getFile("testdata\\tickdb\\misc\\securities5_120303.qsmsg.gz");
        ...
        try {
            ...
            options.distributionFactor = 5;
            options.name = "testDDFSymbols";
            db.createStream(options.name, options);

            final StreamStorage from = new StreamStorage(db, "securities");
            final StreamStorage to = new StreamStorage(db, "testDDFSymbols");

            IdentityKey[] ids = stream.listEntities();
            ReplicationOptions rOptions = new ReplicationOptions();
            rOptions.entities = new IdentityKey[] {ids[0], ids[1], ids[ids.length - 1]};
            new StreamReplicator().replicate(from, to, rOptions);
