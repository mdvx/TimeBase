...
import deltix.util.collections.ArraysComparator;
...
public class Test_SchemaConverter {
    ...
    public File L2_DATA_FILE = Home.getFile("testdata/tickdb/misc/L2-20140710.qsmsg.gz");
    public File L3_DATA_FILE = Home.getFile("testdata/tickdb/misc/5.2/L3-20180704.qsmsg.gz");
    public File L3_DATA_FILE_CHANGE = Home.getFile("testdata/tickdb/misc/5.2/L3-20180704.change.qsmsg.gz");
    ...
    public void Test_EncodingChange() {
        ...
        DXTickStream source = createBarsStream(
                ...
                new TDBRunner.BarsGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100));
    ...
    public void Test_CreateFieldChange3() {
        ...
        try {
            ...
            while (true) {
                if (cursor1.next() && cursor2.next()) {
                    deltix.timebase.api.messages.BarMessage converted = (deltix.timebase.api.messages.BarMessage) cursor1.getMessage();
...
//        ((FieldModifierChange)changes[0]).setInitialValue("122");
    ...
    public void Test_Polymorphic() {
        ...
        dataSource.add(new TDBRunner.TradesGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 10));
        dataSource.add(new TDBRunner.BBOGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 10));
    ...
    public void Test_SeveralTypes() {
        ...
        MessageSourceMultiplexer<InstrumentMessage> dataSource =
                new MessageSourceMultiplexer<InstrumentMessage>();
        dataSource.add(new TDBRunner.TradesGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 10));
        dataSource.add(new TDBRunner.BarsGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 10));
    ...
    public void Test_NoChanges() {
        ...
        DXTickStream source = createStream(tdb, options,
                new TDBRunner.TradesGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, 100));
    ...
    public void testNewCD() throws InterruptedException {        
        ...
        RecordClassDescriptor descriptor =
                StreamConfigurationHelper.mkLevel2MessageDescriptor(
                        null, "", null,
    ...
    public DXTickStream generateTickStream(String name) {
        ...
        dataSource.add(
                new TDBRunner.TradesGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, count, tickers));
        dataSource.add(new TDBRunner.BBOGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, count, tickers));
        dataSource.add(new TDBRunner.BarsGenerator(new GregorianCalendar(2009, 1, 1), (int) deltix.timebase.api.messages.BarMessage.BAR_MINUTE, count, tickers));
    ...
    public void testArrays() throws IOException, InterruptedException {
        ...
        MessageFileHeader header = MessageFileHeader.migrate(Protocol.readHeader(L2_DATA_FILE));

        RecordClassSet classes = new RecordClassSet();
        classes.addClasses(header.getAllTypes());
        classes.addContentClasses(header.getTypes());

        StreamOptions options = StreamOptions.polymorphic(StreamScope.DURABLE, "L2", null, 0);
        options.setMetaData(true, classes);

        DXTickStream etalon = getTickDb().createStream(options.name, options);

        TickDBShell.loadMessageFile(L2_DATA_FILE, etalon);

        RecordClassDescriptor base = StreamConfigurationHelper.mkMarketMessageDescriptor(840);

        RecordClassDescriptor l2snapshot = StreamConfigurationHelper.mkL2SnapshotMessageDescriptor(
                base, null, null, FloatDataType.ENCODING_SCALE_AUTO, FloatDataType.ENCODING_SCALE_AUTO);

        RecordClassDescriptor action =
                StreamConfigurationHelper.mkLevel2ActionDescriptor(
                        FloatDataType.ENCODING_SCALE_AUTO,
                        FloatDataType.ENCODING_SCALE_AUTO
                );

        RecordClassDescriptor l2 = StreamConfigurationHelper.mkLevel2IncrementMessageDescriptor(
                base, null, null, action);


        options = StreamOptions.polymorphic(StreamScope.DURABLE, "l2.new", null, 0, l2snapshot, l2);
        DXTickStream stream = getTickDb().createStream(options.name, options);

        try (TickLoader loader = stream.createLoader(new LoadingOptions(true))) {

            final SchemaConverter converter = createConverter(stream, etalon);

            try (TickCursor cursor = etalon.select(0, new SelectionOptions(true, false), new String[]{deltix.timebase.api.messages.L2Message.class.getName()})) {
                while (cursor.next()) {
                    RawMessage message = (RawMessage) cursor.getMessage();
                    //System.out.println(message);
                    RawMessage changed = converter.convert(message);
        ...
        compareStreams(etalon, stream, new String[]{deltix.timebase.api.messages.L2Message.class.getName()}, etalon.listEntities());
    }

    @Test
    public void testL3() throws Exception {

        MessageFileHeader header = MessageFileHeader.migrate(Protocol.readHeader(L3_DATA_FILE));
        StreamOptions options = StreamOptions.polymorphic(StreamScope.DURABLE, "L3", null, 0, header.getTypes());

        DXTickStream etalon = getTickDb().createStream(options.name, options);
        TickDBShell.loadMessageFile(L3_DATA_FILE, etalon);


        // create stream with changes schema in non-content classes
        header = MessageFileHeader.migrate(Protocol.readHeader(L3_DATA_FILE_CHANGE));
        options = StreamOptions.polymorphic(StreamScope.DURABLE, "l3.new", null, 0, header.getTypes());
        DXTickStream stream = getTickDb().createStream(options.name, options);
        ...
        try (TickLoader loader = stream.createLoader(new LoadingOptions(true))) {
            ...
            try (TickCursor cursor = etalon.select(0, new SelectionOptions(true, false))) {
                while (cursor.next()) {
                    RawMessage message = (RawMessage) cursor.getMessage();
                    //System.out.println(message);
                    RawMessage changed = converter.convert(message);
        ...
        compareStreams(etalon, stream, null, etalon.listEntities());
    }
    ...
    public static void checkEquals(InstrumentMessage msg1, InstrumentMessage msg2)
    {
        ...
        if (msg1 instanceof deltix.timebase.api.messages.BestBidOfferMessage && msg2 instanceof deltix.timebase.api.messages.BestBidOfferMessage) {
            deltix.timebase.api.messages.BestBidOfferMessage bbo1 = (deltix.timebase.api.messages.BestBidOfferMessage) msg1;
            deltix.timebase.api.messages.BestBidOfferMessage bbo2 = (deltix.timebase.api.messages.BestBidOfferMessage) msg2;
        ...
        else if (msg1 instanceof deltix.timebase.api.messages.TradeMessage && msg2 instanceof deltix.timebase.api.messages.TradeMessage) {
            deltix.timebase.api.messages.TradeMessage trade1 = (deltix.timebase.api.messages.TradeMessage) msg1;
            deltix.timebase.api.messages.TradeMessage trade2 = (deltix.timebase.api.messages.TradeMessage) msg2;

            assertEquals(trade1.getPrice(), trade2.getPrice(), 0.00001);
            assertEquals(trade1.getSize(), trade2.getSize(), 0.00001);
        }
        else if (msg1 instanceof deltix.timebase.api.messages.BarMessage && msg2 instanceof deltix.timebase.api.messages.BarMessage) {
            deltix.timebase.api.messages.BarMessage bar1 = (deltix.timebase.api.messages.BarMessage) msg1;
            deltix.timebase.api.messages.BarMessage bar2 = (deltix.timebase.api.messages.BarMessage) msg2;
        ...
        else if (msg1 instanceof deltix.timebase.api.messages.L2Message && msg2 instanceof deltix.timebase.api.messages.L2Message) {
            deltix.timebase.api.messages.L2Message l1 = (deltix.timebase.api.messages.L2Message) msg1;
            deltix.timebase.api.messages.L2Message l2 = (deltix.timebase.api.messages.L2Message) msg2;

            //TODO assertEquals(l1.sequenceId, l2.sequenceId);
            assertEquals(l1.getSequenceNumber(), l2.getSequenceNumber());
            assertEquals(l1.getExchangeId(), l2.getExchangeId());
            assertEquals(l1.isImplied(), l2.isImplied());
            assertEquals(l1.isSnapshot(), l2.isSnapshot());

            assertEquals(0, new ArraysComparator<deltix.timebase.api.messages.Level2ActionInfo, deltix.timebase.api.messages.Level2ActionInfo>() {
                @Override
                public int compare(deltix.timebase.api.messages.Level2ActionInfo a1, deltix.timebase.api.messages.Level2ActionInfo a2) {
                    if (a1 == null && a2 == null)
                        return 0;
                    ...
                    assertEquals(a1.getQuoteId(), a2.getQuoteId());

                    assertEquals(a1.getPrice(), a2.getPrice(), 0.00001);
                    assertEquals(a1.getSize(), a2.getSize(), 0.00001);
                    assertEquals(a1.getQuoteId(), a2.getQuoteId());
                    return 0;
                }
            }.compare(l1.getActions(), l2.getActions()));
        }

        else if (msg1 instanceof deltix.timebase.api.messages.Level2Message && msg2 instanceof deltix.timebase.api.messages.Level2Message) {
            deltix.timebase.api.messages.Level2Message l1 = (deltix.timebase.api.messages.Level2Message) msg1;
            deltix.timebase.api.messages.Level2Message l2 = (deltix.timebase.api.messages.Level2Message) msg2;

            assertEquals(l1.toString(), l1.getSequenceNumber(), l2.getSequenceNumber());
            assertEquals(l1.getSize(), l2.getSize(), 0.00001);
            assertEquals(l1.getPrice(), l2.getPrice(), 0.00001);
            assertEquals(l1.isAsk(), l2.isAsk());
            assertEquals(l1.isLast(), l2.isLast());
            assertEquals(l1.getAction(), l2.getAction());
            assertEquals(l1.getDepth(), l2.getDepth());
            assertEquals(l1.getExchangeId(), l2.getExchangeId());
            assertEquals(l1.getNumOfOrders(), l2.getNumOfOrders());
            assertEquals(l1.getQuoteId(), l2.getQuoteId());
            assertEquals(l1.getCurrencyCode(), l2.getCurrencyCode());
        }
    }
}
