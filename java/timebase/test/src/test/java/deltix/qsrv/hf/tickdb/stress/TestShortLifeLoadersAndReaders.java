package deltix.qsrv.hf.tickdb.stress;
 ...
 *
 * All loaders and readers share single TB client.
 *
 * @author Alexei Osipov
 */
// Stress test - don't run in generic build
public class TestShortLifeLoadersAndReaders extends BaseStressTest {
    ...
    public void test() throws InterruptedException {
        ...
        try {
            for (int i = 0; i < STREAM_COUNT; i++) {
                DXTickStream stream = createTestStream("test" + i);
                streams.add(stream);
                AtomicLong loaderCounter = new AtomicLong();
                AtomicLong consumerCounter = new AtomicLong();
                threads.add(createLoaderThread(stream, loaderCounter));
                threads.add(createConsumerThread(stream, consumerCounter));
    ...
    private Thread createConsumerThread(DXTickStream stream, AtomicLong consumerCounter) {
        return new Thread(() -> {
            Random rnd = new Random();
            int count = 0;
            while (!Thread.currentThread().isInterrupted()) {
                final TickCursor cursor = stream.select(Long.MIN_VALUE, new SelectionOptions(true, false));
                int targetCursorMessages = rnd.nextInt(200);
    ...
    private Thread createLoaderThread(DXTickStream stream, AtomicLong loaderCounter) {
        return new Thread(() -> {
            int count = 0;
            Random rnd = new Random();
