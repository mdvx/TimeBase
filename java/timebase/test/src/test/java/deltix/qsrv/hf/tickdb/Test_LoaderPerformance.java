    ...
    public void test() {
        ...
        if (stream == null)
            stream = runner.getTickDb().createStream("test",
                    StreamOptions.polymorphic(StreamScope.DURABLE, "test", null, DF, bbo, trade, l2));
...
class L2Generator extends BaseGenerator<InstrumentMessage> {
    private deltix.timebase.api.messages.Level2Message message = new deltix.timebase.api.messages.Level2Message();
    protected int count;

    L2Generator(GregorianCalendar calendar, int interval, String... tickers) {
        super(calendar, interval, tickers);

        message.setPrice(nextDouble());
        ...
        message.setNumOfOrders(0);
    }

    @Override
    public boolean next() {

        message.setIsLast(count % 10 == 0);
        message.setSymbol(symbols.next());
        message.setTimeStampMs(symbols.isReseted() ? getNextTime() : getActualTime());
